...
library, with the Julia runtime **embedded** in the bundle. From the caller's
point of view it is an ordinary C library: there is no Julia process to start
and no `.jl` file to ship — only `libkrylov.so` (or `.dylib` / `.dll`) and the
two headers `krylov.h` / `krylov.f90`.
...
- **Concrete types, no dynamic dispatch on the hot path.** A workspace is
  created for one `(solver, precision)` pair, so every product, dot, and axpy
  runs through statically-typed, fully specialised Julia code — the same code
  paths you get from Julia itself.
...
case (CG, GMRES, …). The distinction matters for least-squares / least-norm
solvers where `A` is rectangular.
...
of that element type. The complex layout is the standard interleaved
real/imaginary one, identical to C99 `_Complex` and to Julia's `Complex{T}`, so
no conversion is needed. The only device currently available is
`KRYLOV_CPU = 0`.
...
Pass a `matvec_M` that applies `M⁻¹` (i.e. computes `y = M \ x`) to use `M` as a
left preconditioner; pass `NULL` for an unpreconditioned solve. The callback
sees vectors of length `n` and, like the operator, is matrix-free — `M` can be
an incomplete factorization, a sparse approximate inverse, a multigrid V-cycle,
or anything else you can evaluate. See [Preconditioners](@ref preconditioners)
for guidance on choosing one.
...
runtime into a relocatable bundle. The full, copy-pasteable build commands —
plus the `gcc` / `gfortran` invocations and the platform-specific `-rpath`
flags for Linux, macOS and Windows — live in
[`interfaces/README.md`](https://github.com/JuliaSmoothOptimizers/Krylov.jl/blob/main/interfaces/README.md).
Pre-built bundles are attached to each
[release](https://github.com/JuliaSmoothOptimizers/Krylov.jl/releases).
