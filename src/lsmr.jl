...
LSMR can be warm-started from an initial guess `x0` with the method

    (x, stats) = lsmr(A, b, x0; kwargs...)
...
function lsmr(A, b :: AbstractVector{FC}, x0 :: AbstractVector; window :: Int=5, kwargs...) where FC <: FloatOrComplex
  solver = LsmrSolver(A, b, window=window)
  lsmr!(solver, A, b, x0; kwargs...)
...
"""
    ...
    solver = lsmr!(solver::LsmrSolver, A, b, x0; kwargs...)
...
function lsmr!(solver :: LsmrSolver{T,FC,S}, A, b :: AbstractVector{FC}, x0 :: AbstractVector; kwargs...) where {T <: AbstractFloat, FC <: FloatOrComplex{T}, S <: DenseVector{FC}}
